              ...
              itemBuilder: (context, index) {
                ...
                return Card(
                  ...
                  child: ListTile(
                    ...
                    leading: _buildReceitaImagem(receita),
                    title: Text(receita.nome, style: const TextStyle(fontWeight: FontWeight.bold)),
                    subtitle: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        const SizedBox(height: 4),
                        Text('Rendimento: ${receita.rendimento} ${receita.unidadeRendimento}'),
                        Text(
                          'Preço por ${receita.unidadeRendimento}: ${MoneyFormatter.formatReal(receita.valorPorUnidade)}',
                    ...
                    onTap: () => context.push('/receitas/detalhes/${receita.id}'),
                  ),
                );
              },
            ),
      floatingActionButton: FloatingActionButton(
        onPressed: () => context.push('/receitas/nova'),
        backgroundColor: Theme.of(context).colorScheme.primary,
        foregroundColor: Colors.white,
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const Text('Nenhuma receita cadastrada', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
          const SizedBox(height: 8),
          const Text('Adicione receitas para calcular os preços', style: TextStyle(color: Colors.grey)),
      ...
      try {
        return ClipRRect(
          ...
          child: SizedBox(
            width: 50,
            height: 50,
            child: receita.imagemUrl!.startsWith('http')
                ? Image.network(
                    receita.imagemUrl!,
                    fit: BoxFit.cover,
  ...
  void _excluirReceita(String id) {
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Receita excluída com sucesso'), backgroundColor: Colors.green));
    } catch (e) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Erro ao excluir receita: $e'), backgroundColor: Colors.red));
    }
  }
}
